...
interface IGraph<V> {
     ...
     *	method shall only return true if there exists a self-edge on * the specified vertex.
     *
     ...
     *
     *	Changes to the returned Iterable object (e.g., .remove()) * shall NOT impact or change the graph.
     *
     *	@param vertexName The target vertex
     *	@return An iterable, possibly empty, containing all * neighboring vertices.
     *	@throws java.util.NoSuchElementException if the vertex is not
     *	present in the graph
     */
    Iterable<V> neighbors(V vertexName);
    /**
     *	Deletes all trace of the specified vertex from within the * graph.
     *
     *	This method deletes the vertex from the graph as well as every * edge using the specified vertex as a start (out) or * destination (in) vertex.
     *
    ...
    /**
     *	Returns one shortest path through the graph from the starting * vertex and ending in the destination vertex.
     *
     *	@param start	The vertex from which to begin the search
     *	@param destination The terminal vertex within the graph
     *	@return A sequence of vertices to visit requiring the fewest * steps through the graph from its starting position
     *	(at index 0 in the list) to its terminus at the list's end. * If no path exists between the nodes, this method * returns an empty list.
     *	@throws java.util.NoSuchElementException if either vertex are not present in the graph
     */
    ...
    Iterable<V> vertices();
    /**
     *	Produces a graph of only those vertices and edges reachable * from the origin vertex.
     *
     *	@param origin The vertex to build the graph from
     *	@return A new graph with only the Vertices and Edges * reachable from the parameter Vertex.
     *	@throws java.util.NoSuchElementException if the origin vertex
     *	is not present in this graph
     */
